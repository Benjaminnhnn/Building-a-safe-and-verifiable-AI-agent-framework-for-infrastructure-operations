...
class IncidentStateMachine:
    ALLOWED_TRANSITIONS: ClassVar[
        ...
        IncidentStatus.TRIAGED: frozenset({IncidentStatus.PLANNED, IncidentStatus.FAILED}),
        IncidentStatus.PLANNED: frozenset({IncidentStatus.GATED, IncidentStatus.FAILED}),
        IncidentStatus.GATED: frozenset({IncidentStatus.EXECUTED, IncidentStatus.ESCALATED, IncidentStatus.FAILED}),
        IncidentStatus.ESCALATED: frozenset({IncidentStatus.PLANNED, IncidentStatus.FAILED}),
        IncidentStatus.EXECUTED: frozenset({IncidentStatus.VERIFYING, IncidentStatus.FAILED}),
        IncidentStatus.VERIFYING: frozenset({IncidentStatus.RESOLVED, IncidentStatus.FAILED}),
    ...
    def transition(
        ...
        *,
        ...
        if requested_state == IncidentStatus.RESOLVED and actor != "independent_verifier":
        ...
        event_id = "audit-" + hashlib.sha256(event_seed.encode("utf-8")).hexdigest()[:16]
        event = AuditEvent(
            event_id=event_id,
